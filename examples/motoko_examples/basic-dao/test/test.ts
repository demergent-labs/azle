...
const tests: Test[] = [
  ...
  },
  {
    ...
    test: async () => {
      ...
      assert(bobBalance.amount_e8s === 0n, "Bob's account should be empty");
      ...
      if ('err' in transfer) {
        assert(
          transfer.err === 'Caller needs an account to transfer funds',
  ...
  },
  {
    ...
    test: async () => {
      ...
      assert(systemParams.transfer_fee.amount_e8s === 0n, 'Should have 0 fee');
  ...
  },
  {
    ...
    test: async () => {
      ...
      assert(
        'err' in voteEve &&
          voteEve.err.includes('Caller does not have any tokens to vote with'),
        'Should have an error'
      );

      const [{ id, payload, proposer, state, voters, votes_no, votes_yes }] =
