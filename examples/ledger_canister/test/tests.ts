...
function getSimpleTests(ledgerCanister: ActorSubclass<_SERVICE>): Test {
    return () => {
        ...
        it('getBlocks', async () => {
            ...
            if (transfer_1 === undefined || !('Mint' in transfer_1)) {
                throw new Error();
            }
            if (transfer_2 === undefined || !('Transfer' in transfer_2)) {
                throw new Error();
            }
            if (transfer_3 === undefined || !('Transfer' in transfer_3)) {
                throw new Error();
            }

            expect(transfer_1.Mint.amount.e8s).toBe(100_000_000_000n);
            expect(transfer_2.Transfer.amount.e8s).toBe(2_000_000n);
            expect(transfer_3.Transfer.amount.e8s).toBe(3_000_000n);
        }, 10_000);
