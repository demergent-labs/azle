...
export default class {
    ...
    @update
    async execute_create_canister(): Promise<ExecuteCreateCanisterResult> {
        const create_canister_result_canister_result = await management_canister
            .create_canister({
                settings: null
            })
        ...
        const create_canister_result =
            create_canister_result_canister_result.ok;

        this.state.created_canister_id = create_canister_result.canister_id;
    ...
    async get_canister_status(
        ...
        const canister_status_result =
            canister_status_result_canister_result.ok;
    ...
    @update
    async get_raw_rand(): Promise<RawRandResult> {
        const raw_rand_canister_result = await management_canister
            .raw_rand()
            .call();
    ...
    // TODO we will test these once we can measure cycles better locally
    @update
    async provisional_create_canister_with_cycles(): Promise<ExecuteProvisionalCreateCanisterWithCyclesResult> {
        const canister_result = await management_canister
            .provisional_create_canister_with_cycles({
                amount: null,
                settings: null
        ...
        const provisional_create_canister_with_cycles_result =
            canister_result.ok;
    ...
    // TODO we will test these once we can measure cycles better locally
    @update
    async provisional_top_up_canister(
        canister_id: Principal,
        amount: nat
    ): Promise<DefaultResult> {
        const canister_result = await management_canister
    ...
    @query
    get_created_canister_id(): Principal {
        return this.state.created_canister_id;
    }
}
