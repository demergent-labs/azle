...
gzip -9 "$HOME/.config/azle/azle_${VERSION}_image.tar"
...
if [[ "$VERSION" == *"-rc."* ]];
then
    gh release create "$VERSION" "$HOME/.config/azle/azle_${VERSION}_image.tar.gz" -t "$VERSION" --prerelease
else
    gh release create "$VERSION" "$HOME/.config/azle/azle_${VERSION}_image.tar.gz" -t "$VERSION"
fi
