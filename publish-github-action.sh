...
gzip -9 "~/.config/azle/azle_${VERSION}_image.tar"
...
if [[ "$VERSION" == *"-rc."* ]];
then
    gh release create "$VERSION" "~/.config/azle/azle_${VERSION}_image.tar.gz" -t "$VERSION" --prerelease
else
    gh release create "$VERSION" "~/.config/azle/azle_${VERSION}_image.tar.gz" -t "$VERSION"
fi
