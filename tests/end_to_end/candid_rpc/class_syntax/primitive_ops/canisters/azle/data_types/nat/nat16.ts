import { ic, nat16, nat32, update } from 'azle/experimental';

import { PerfResult } from '../../perf_result';

let nat16_init_heap_storage: { [key: string]: nat16 | undefined } = {};

export const nat16_init_stack = update([nat32], PerfResult, (num_inits) => {
    const perf_start = ic.performanceCounter(0);

    let i = 0;

    while (i < num_inits) {
        let _value: nat16 = i % 2 === 0 ? 65_535 : 0;
...
export const nat16_init_heap = update([nat32], PerfResult, (num_inits) => {
    const perf_start = ic.performanceCounter(0);
