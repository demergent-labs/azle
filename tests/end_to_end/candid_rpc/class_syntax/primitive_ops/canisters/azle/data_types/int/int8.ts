import { ic, int8, nat32, update } from 'azle/experimental';

import { PerfResult } from '../../perf_result';

let int8_init_heap_storage: { [key: string]: int8 | undefined } = {};

export const int8_init_stack = update([nat32], PerfResult, (num_inits) => {
    const perf_start = ic.performanceCounter(0);

    let i = 0;

    while (i < num_inits) {
        let _value: int8 = i % 2 === 0 ? 127 : 0;
...
export const int8_init_heap = update([nat32], PerfResult, (num_inits) => {
    const perf_start = ic.performanceCounter(0);
