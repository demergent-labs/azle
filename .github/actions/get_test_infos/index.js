...
const [, , sortedDirsArg, excludeDirsArg] = process.argv;

// Convert comma-separated strings to arrays
const sortedDirs = sortedDirsArg.split(',').filter((path) => path !== '');
const excludeDirs = excludeDirsArg.split(',').filter((path) => path !== '');

async function main() {
    try {
        const result = await processDirs(sortedDirs, excludeDirs);
