...
function wasiFsSyscallErrorMap(err, syscall, path, dest) {
    ...
    let code =
        codeToErrorMsg[err.code] === undefined ? 'E' + err.code : err.code;
...
function rmdirSync(
    path,
    options = { maxRetries: 0, recursive: false, retryDelay: 100 }
) {
    path = getValidatedPath(path);
...
function rmSync(
    path,
    options = { force: false, maxRetries: 1, recursive: false, retryDelay: 100 }
) {
    path = getValidatedPath(path);
    validateObject(options, options);
...
function read(fd, buffer, offset, length, position, callback) {
    if (typeof buffer === 'function') {
        ...
        let option = buffer ?? {};
        buffer =
            option.buffer !== undefined ? option.buffer : Buffer.alloc(16384);
    ...
    if (typeof position === 'bigint') {
        if (position < nodePositionMin || position > nodePositionMax) {
            throw new errors.ERR_OUT_OF_RANGE(
                'position',
                `>= ${nodePositionMin} && <= ${nodePositionMax}`,
                position
            );
        } else {
...
function readv(fd, buffer, position, callback) {
    ...
    if (
        ...
        (buffer.length > 0 && !isArrayBufferView(buffer[0]))
    ) {
        throw new errors.ERR_INVALID_ARG_TYPE(
            'buffers',
            'ArrayBufferView[]',
...
function writeFile(file, data, options, callback) {
    ...
    let buffer =
        typeof data === 'string' ? Buffer.from(data, options.encoding) : data;
    if (
        !(buffer instanceof Buffer) &&
...
class Dirent {
    ...
    isSocket = () =>
        this.filetype === FILETYPE_SOCKET_DGRAM ||
        this.filetype === FILETYPE_SOCKET_STREAM;
}
    ...
    #fetch() {
        ...
        if (this.#idx === this.#dataBuf.length && !this.#fin) {
            try {
                ...
                this.#dataBuf.push(
                    ...data.res.filter((d) => d.name !== '.' && d.name !== '..')
                );
                this.#fin = data.fin;
...
const validateCpOptions = hideStackFrames((options) => {
    ...
    if (options.dereference === true && options.verbatimSymlinks === true) {
        throw new errors.ERR_INCOMPATIBLE_OPTION_PAIR(
            'dereference',
            'verbatimSymlinks'
        );
    }
