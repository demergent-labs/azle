...
r.prototype.parse = function (t, s, h) {
    ...
    if (!g[j] && (x || (j && !y[j]))) {
        ...
        if (!U) {
            for (
                var k = this.hostname.split(/\./), S = ((w = 0), k.length);
                w < S;
                w++
            ) {
                var R = k[w];
...
r.prototype.resolveObject = function (t) {
    ...
    if (
        ...
        ((h.host = t.host || '' === t.host ? t.host : h.host),
            (h.hostname =
                t.hostname || '' === t.hostname ? t.hostname : h.hostname),
            (h.search = t.search),
            (h.query = t.query),
            (x = m));
    } else if (m.length) {
...
function fileURLToPath$1(path) {
    ...
    return isWindows$1
        ? getPathFromURLWin$1(path)
        : getPathFromURLPosix$1(path);
}
...
function pathToFileURL$1(filepath) {
    ...
    if (
        (filePathLast === CHAR_FORWARD_SLASH$1 || isWindows$1) &&
...
function pathToFileURL(filepath) {
    ...
    var filePathLast = filepath.charCodeAt(filepath.length - 1);
    if (
        (filePathLast === CHAR_FORWARD_SLASH || isWindows) &&
