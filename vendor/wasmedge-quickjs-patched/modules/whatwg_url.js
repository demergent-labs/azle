...
var extraFragmentPercentEncodeSet = new Set([
    p$1(' '),
    p$1('"'),
    p$1('<'),
    p$1('>'),
...
var extraQueryPercentEncodeSet = new Set([
    p$1(' '),
    p$1('"'),
    p$1('#'),
    p$1('<'),
...
var extraComponentPercentEncodeSet = new Set([
    p$1('$'),
    p$1('%'),
    p$1('&'),
    p$1('+'),
...
var extraURLEncodedPercentEncodeSet = new Set([
    p$1('!'),
    p$1("'"),
    p$1('('),
    p$1(')'),
...
(function (module) {
    ...
    function isWindowsDriveLetterString(string) {
        return (
            string.length === 2 &&
            infra$1.isASCIIAlpha(string.codePointAt(0)) &&
            (string[1] === ':' || string[1] === '|')
        );
    }
    ...
    URLStateMachine.prototype['parse authority'] = function parseAuthority(
        ...
        if (c === p('@')) {
            ...
            for (var pointer = 0; pointer < len; ++pointer) {
                ...
                var encodedCodePoints = utf8PercentEncodeCodePoint(
                    codePoint,
    ...
    URLStateMachine.prototype['parse query'] = function parseQuery(c, cStr) {
        ...
        if ((!this.stateOverride && c === p('#')) || isNaN(c)) {
            var queryPercentEncodePredicate = isSpecial(this.url)
    ...
    module.exports.setThePassword = function (url, password) {
        url.password = utf8PercentEncodeString(
            password,
            isUserinfoPercentEncode
        );
    };
...
function serializeUrlencoded(tuples, encodingOverride) {
    ...
    for (var [i, tuple] of tuples.entries()) {
        ...
        var name = utf8PercentEncodeString(
            tuple[0],
