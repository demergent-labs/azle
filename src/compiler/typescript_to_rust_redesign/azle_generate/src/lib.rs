...
use generators::canister_methods::{
    generate_query_function_infos, generate_update_function_token_streams,
    get_ast_fn_decls_from_programs, get_ast_type_alias_decls_from_programs, get_query_fn_decls,
...
pub fn azle_generate(ts_file_names_token_stream: TokenStream) -> TokenStream {
    ...
    let query_function_token_streams: Vec<proc_macro2::TokenStream> = query_function_info
        .iter()
        .map(|fun_info| fun_info.token_stream.clone())
        .collect();

    // let query_inline_type_aliases = quote!();
    let (update_function_info, count) =
        generate_update_function_token_streams(&ast_fnc_decls_update, inline_dep_count);
    inline_dep_count = count;
