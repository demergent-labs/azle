...
pub fn generate() -> TokenStream {
    quote! {
        fn call_raw<'a>(
            ...
            let promise_id: String = args.get(0).expect("call_raw promise_id argument is undefined").to_js_value()?.try_into()?;
            let canister_id_bytes: Vec<u8> = args.get(1).expect("call_raw canister_id_bytes is undefined").to_js_value()?.try_into()?;
            ...
            ic_cdk::spawn(async move {
                ...
                CONTEXT.with(|context| {
                    ...
                    if should_resolve {
                        let resolve = global.get_property(format!("_resolve_{promise_id}").as_str()).unwrap();
                        resolve.call(&resolve, &[js_value_ref]).unwrap();
                    }
                    else {
                        let reject = global.get_property(format!("_reject_{promise_id}").as_str()).unwrap();
