...
pub fn generate() -> TokenStream {
    quote! {
        fn notify_raw<'a>(
            ...
            let canister_id_bytes: Vec<u8> = args.get(0).expect("notify_raw canister_id_bytes is undefined").to_js_value()?.try_into()?;
            let canister_id = candid::Principal::from_slice(&canister_id_bytes);
            let method: String = args.get(1).expect("notify_raw method argument is undefined").to_js_value()?.try_into()?;
            let args_raw: Vec<u8> = args.get(2).expect("notify_raw args_raw argument is undefined").to_js_value()?.try_into()?;
            let payment_candid_bytes: Vec<u8> = args.get(3).expect("notify_raw payment_candid_bytes argument is undefined").to_js_value()?.try_into()?;
