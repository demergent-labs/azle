...
impl AzleTypeRef<'_> {
    ...
    fn variant_wrong_enclosed_type_error(&self, source_map: &SourceMap) -> String {
        let well_formed = source_map.get_well_formed_line(self.ts_type_ref.span);
        let example = self.generate_example_variant();
        let example_variant = format!("\n{}{}\n", well_formed, example);
        let location = source_map.get_line_info(self.ts_type_ref.span);
        let highlighted_line = source_map.generate_highlighted_line(self.ts_type_ref.span);
