...
pub fn generate(methods: &Vec<QueryOrUpdateMethod>) -> TokenStream {
    ...
    quote! {
        fn async_await_result_handler(
            ...
            .map_err(|js_error| js_error.to_console_string(boa_context))?;
            ...
            return match &state {
                ...
                boa_engine::builtins::promise::PromiseState::Rejected(js_value) => {
                    ...
                    return Err(js_value.clone().to_console_string(boa_context));
                }
    ...
    quote!(
        #name => {
            let reply_value: (#return_type) = js_value
                ...
                .map_err(|js_error: boa_engine::JsError| js_error.to_console_string(boa_context))?;
